...
create or replace function get_process_name_by_id (
  i_process_id number
) return varchar2
is
...
/
create or replace function get_parameter_name_by_id (
...
create or replace function get_setting_type_from_value_type(
...
begin
    ...
    for record in (select setting_id, is_active, value_type from settings where PARAMETER_ID = l_parameter_id and PROCESS_ID = l_process_id)
    loop
...
begin
    for record in (
        select get_parameter_name_by_id(parameter_id) as parameter_name,
               get_process_name_by_id(process_id) as process_name
        from settings group by (parameter_id, process_id)
        ) loop
            select * into result from Table(render_task_8(record.parameter_name,record.process_name));
        end loop;
end;
/
select * from Table(render_task_8(get_parameter_name_by_id(3),get_process_name_by_id(2)))
/