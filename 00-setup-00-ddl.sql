...
-- OPTIMIZATION: can we get rid of the sequence and `parameter_id` field?
...
-- OPTIMIZATION: index these as numbers instead?.
...
-- create a sequence for auto generating setting_id
create sequence settings_seq;

-- create table settings to store (parameter_id, process_id, is_active, value_type_ fields.
-- This table will get huge in no time and can be normalized.
...
-- create a table to store values for scalar settings
...
-- add a foreign key constraint on the setting_id field
...
-- create a table to store values for function setting types
...
-- sequence for generating log_ids
...
-- custom type represents a table of number in memory.
