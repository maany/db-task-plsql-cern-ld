...
begin
  ...
  insert into log_messages (message) values ('create_int_setting: i_parameter_name='||i_parameter_name
    ||'; i_process_name='||i_process_name||'; i_value='||i_value);   

  update settings
  set is_active = 'N'
  where is_active = 'Y' and parameter_id = get_parameter_id_by_name(i_parameter_name) and process_id = get_process_id_by_name(i_process_name);
  
  insert into settings (setting_id, parameter_id, process_id, is_active, value_type)
  values (settings_seq.nextval, get_parameter_id_by_name(i_parameter_name), get_process_id_by_name(i_process_name), 'Y', 'INT')
...
begin
  ...
  insert into log_messages (message) values ('copy_settings: i_src_parameter_name='||i_src_parameter_name
    ||'; i_src_process_name='||i_src_process_name||'; i_dest_parameter_name='||i_dest_parameter_name||'; i_dest_process_name='||i_dest_process_name);
